...
export default function DeleteEmployeeDialog({ stock }: any) {
  ...
  const supplyData = stock;
  const { deleteMainSupply } = useMainSupplies();
  const router = useRouter();

  async function onSubmit(dataProps?: any) {
    startTransition(async () => {
      const result = await deleteMainSupply(dataProps, 1000);
      const { error } = result;
      if (error?.message) {
        toast({
          variant: "destructive",
      ...
      router.push("/application/main_supplies");
    });
  }
