...
import { DataTable as MainSuppliesCart } from "./add-return-cart/main-supplies-cart/data-table";

import { initiateColumns as initiateMainSuppliesCartColumns } from "./add-return-cart/main-supplies-cart/columns";
...
import MainSuppliesCartOptions from "./add-return-table/lists";
import { useDispatch } from "react-redux";
import { resetCart } from "@/redux/slices/requestCartSlice";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
export default function ReturnForm({ setDialogOpen }: any) {
  ...
  form.setValue("restock_supplies", requestCart);
  useEffect(() => {
    form.setValue(
      "total_price",
      requestCart.reduce(
  ...
  async function onSubmit(data: any) {
    startTransition(async () => {
      const result = await createReturn(data, 500);
      ...
      >
          ...
          <ScrollArea className="w-full h-[553px] 2xl:h-[657px] flex flex-col justify-between bg-white rounded-lg border border-lightBorder p-0 px-4 gap-0 relative">
            <div className="w-full h-full flex flex-col gap-6 justify-between relative">
              ...
              <div className="w-full py-6 flex gap-8 position sticky bottom-[-4px] bg-white m-0 text-lg font-bold">
                ...
                <span className="w-[20%] text-end">{`₱ ${requestCart
                  .reduce(
                    (acc: any, supply: any) =>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            type="submit"
            disabled={
              requestCart.length === 0 && requestCart.length === 0
                ? true
                : false
            }
          >
