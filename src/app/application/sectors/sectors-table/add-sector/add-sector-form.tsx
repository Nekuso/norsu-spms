...
export default function SectorForm({ setDialogOpen }: any) {
  const [isPending, startTransition] = useTransition();
  const { createSector } = useSectors();

  const sectortScheema = z.object({
    sector_name: z.string().min(3, "Name is too short"),
    sector_type: z.string().min(1, "Sector is required"),
    description: z.string().min(3, "Description is too short"),
  });
  const form = useForm<z.infer<typeof sectortScheema>>({
    resolver: zodResolver(sectortScheema),
  });

  async function onSubmit(data: any) {
    startTransition(async () => {
      const result = await createSector(data, 1500);
      const { error } = result;
      if (error?.message) {
        toast({
          variant: "destructive",
      ...
      >
        ...
        <DialogFooter>
          <Button
            ...
            type="submit"
          >
            <span className={cn({ hidden: isPending })}>Create Sector</span>
